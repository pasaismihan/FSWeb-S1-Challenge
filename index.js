...
console.log(fenomenler[4].profile);
...
function indekseGoreFenomen(first, second) {
  return `${first.indexOf(first[second])}. indekste bulunan fenomen: ${
...
function profilListesi(first) {
  let yeniArray = [];
  for (let i = 0; i < first.length; i++) {
    yeniArray.push(first[i].profile);
  }
  return yeniArray;
}
...
function fenomenSil(first, dizi) {
  let yeniObj = [...first];
  yeniObj.splice(dizi, 1);
  return yeniObj;
}
...
function fenomenEkle(feno, first, second, third, fourth, fifth) {
  let newDizi = [...feno];
  let yeniArrr = {
    number: first,
    profile: second,
...
function fenomenGonderimSayisi(feno, profilAl) {
  for (let i = 0; i < feno.length; i++) {
    if (feno[i].profile == profilAl) {
      return feno[i].posts;
    }
  }
}
...
function platformaGoreCokGonderiYapanFenomen(fenomenler, platform) {
  ...
  for (let i = 0; i < fenomenler.length; i++) {
    if (
      fenomenler[i].platform == platform &&
